/**
 * TenpaiChecker - 聴牌判定ユーティリティ（クライアント側）
 * サーバー側のロジックと同じアルゴリズムを使用
 */

export interface Tile {
  suit: string; // 'man', 'pin', 'sou', 'honor'
  number: number; // 1-9 for suits, 1-7 for honors
  display?: string;
}

export interface WinningTile extends Tile {
  display: string;
  count: number; // 壁に残っている枚数
}

export interface TenpaiResult {
  isTenpai: boolean;
  winningTiles: WinningTile[];
}
...
export class TenpaiChecker {
   ...
   * タイルを比較してソート順序を決定
   */
  static compareTiles(a: Tile, b: Tile): number {
   ...
   * タイルの等価性を判定
   */
  static tileEquals(tile1: Tile, tile2: Tile): boolean {
    return tile1.suit === tile2.suit && tile1.number === tile2.number;
  }

  /**
   * 有効なメルド構造をチェック（再帰的に複数の組み合わせを試行）
   * @param tiles - チェック対象のタイル配列
   * @returns 有効な構造ならtrue
   */
  static checkValidMeldStructure(tiles: Tile[]): boolean {
    // 2枚以上必要（最小限のペア）
   ...
   * @param tiles - チェック対象のタイル配列
   * @returns 有効な組が形成できるならtrue
   */
  static canFormSets(tiles: Tile[]): boolean {
   ...
   * @param hand - 手牌のタイル配列
   * @param melds - 副露済みの組（組は3枚のタイル配列）
   * @returns 和了可能なタイルのリスト
   */
  static getWinningTiles(hand: Tile[], melds: Tile[][]): WinningTile[] {
    const winningTiles: WinningTile[] = [];
    const tileCountInHand: Record<string, number> = {};
    ...
    const numbers: Record<string, number> = { 'man': 9, 'pin': 9, 'sou': 9, 'honor': 7 };
    ...
    suits.forEach((suit) => {
      for (let i = 1; i <= numbers[suit]; i++) {
        ...
        if (tileCount < 4) {
          ...
          const testTile: Tile = { suit, number: i };
   ...
   */
  static getChiitoitsuWinningTiles(hand: Tile[], tileCountInHand: Record<string, number>): WinningTile[] {
    if (hand.length !== 13) {
      return [];
    }
   ...
   */
  static isKokushi(hand: Tile[]): boolean {
    if (hand.length !== 14) return false;
   ...
   */
  static getKokushiWinningTiles(hand: Tile[], tileCountInHand: Record<string, number>): WinningTile[] {
    if (hand.length !== 13) {
      return [];
    }
   ...
   * タイルの表示文字を取得
   */
  static getTileDisplay(suit: string, number: number): string {
    ...
    const numberChar: Record<string, string> = {
      '1': '一',
      '2': '二',
      '3': '三',
      '4': '四',
   ...
   * 牌を捨てた後に聴牌しているかチェック
   * @param hand - 手牌のタイル配列
   * @param tileIndex - 捨てる牌のインデックス
   * @param melds - 副露済みの組
   * @returns { isTenpai: boolean, winningTiles: Array }
   */
  static checkTenpaiAfterDiscard(hand: Tile[], tileIndex: number, melds: Tile[][]): TenpaiResult {
   ...
   * @param hand - 手牌のタイル配列
   * @param melds - 副露済みの組
   * @returns { [tileIndex]: { isTenpai, winningTiles } }
   */
  static checkAllTenpai(hand: Tile[], melds: Tile[][]): Record<number, TenpaiResult> {
    const results: Record<number, TenpaiResult> = {};
